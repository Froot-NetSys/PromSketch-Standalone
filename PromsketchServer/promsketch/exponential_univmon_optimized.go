...
// UpdateWindow updates the time window size for the Univ sketch
...
func (ehu *ExpoHistogramUnivOptimized) Update(time_ int64, fvalue float64) {
	...
	if 2*len(ehu.map_buckets[0].m) >= ehu.max_map_size {
		...
		// If the number of elements in the map becomes very large
		// the corresponding bucket is converted into a UnivSketch.
		// Merge EH buckets into (univmon)
...
// checks if the data in the EH histogram covers a certain time range [mint, maxt]
...
func (ehu *ExpoHistogramUnivOptimized) QueryIntervalMergeUniv(t1, t2 int64, cur_t int64) (univ *UnivSketch, m *map[float64]int64, n float64, cacheHit bool) {
	...
	if to_bucket < ehu.s_count {
		...
		if from_bucket < to_bucket {
			...
			ehu.mutex.RUnlock()
			return merged_univ, nil, 0, true
		} else {
			ehu.mutex.RUnlock()
			return ehu.univs[from_bucket], nil, 0, true
		}
		...
		ehu.mutex.RUnlock()
		return merged_univ, nil, 0, false
	}
}
