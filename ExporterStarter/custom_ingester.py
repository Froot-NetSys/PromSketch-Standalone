...
# Assumes you're using InfluxDB; install with: pip install influxdb-client
from influxdb_client import InfluxDBClient, Point, WriteOptions
from influxdb_client.client.write_api import SYNCHRONOUS

# InfluxDB configuration (replace with your credentials)
...
def ingest_data(config_file):
    ...
    while True: # Loop to continually scrape data
        for target in targets:
            try:
                # Send an HTTP GET request to the exporter
                response = requests.get(f"http://{target}/metrics")
                response.raise_for_status() # Handle HTTP errors
                
                # Parse the Prometheus metrics response
                for family in text_fd_to_metric_families(StringIO(response.text)):
                    for sample in family.samples:
                        ...
                        point.time(time.time_ns()) # Use nanoseconds for high precision
        ...
        # Sleep according to scrape_interval from the config file (or default)
